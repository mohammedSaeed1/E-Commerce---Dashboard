...
const BASE_URL = "https://nti-ecommerce.vercel.app/api/v1/subCategories"
...
export default function SubCategories() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      category: Yup.string().required("Category is required").length(24, "Invalid category ID")
    }),
    onSubmit: (values) => {
  ...
  function addCategory(values) {
    ...
    axios.post(BASE_URL, values)
      .then(() => { getAllSubCategories(); closeModal() })
      .catch((err) => console.error(err))
  }

  function updateCategory(values) {
    axios.put(`${BASE_URL}/${selectedSubCategory._id}`, { name: values.name })
      .then(() => { getAllSubCategories(); closeModal() })
      .catch((err) => console.error(err))
  }

  function deleteCategory() {
    axios.delete(`${BASE_URL}/${selectedSubCategory._id}`)
      .then(() => { getAllSubCategories(); setShowDeleteModal(false) })
      .catch((err) => console.error(err))
  }

  useEffect(() => { getAllSubCategories() }, [])
    ...
    <div className="bg-[#1b262c] min-h-screen p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2 bg-[rgba(50,130,184,0.1)] border border-[rgba(50,130,184,0.2)] rounded-full px-3.5 py-1.5">
          ...
          <span className="text-[12px] font-semibold text-[#3282b8]">{subCategories.length} SubCategories</span>
        </div>
      </div>
      ...
      {!loading && subCategories.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-[14px] text-[rgba(187,225,250,0.35)]">No subcategories yet</p>
          <div onClick={openAdd} className="text-[13px] text-[#3282b8] cursor-pointer hover:underline">+ Add your first subcategory</div>
        </div>
      )}
        ...
        <div onClick={closeModal} className="fixed inset-0 bg-[rgba(15,30,40,0.85)] flex items-center justify-center z-50 px-4">
          <div onClick={(e) => e.stopPropagation()} className="bg-[#0f4c75] border border-[rgba(50,130,184,0.3)] rounded-2xl w-full max-w-sm max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5">
              ...
              {/* Update banner */}
              {mode === "update" && selectedSubCategory && (
                <div className="flex items-center gap-3 bg-[rgba(27,38,44,0.5)] border border-[rgba(50,130,184,0.15)] rounded-xl p-3 mb-4">
                  ...
                  <div className="min-w-0">
                    <div className="text-[13px] font-semibold text-[#bbe1fa] truncate">{selectedSubCategory.name}</div>
                    <div className="text-[11px] text-[rgba(187,225,250,0.35)] mt-0.5">Editing this subcategory</div>
                  </div>
                </div>
              )}
                ...
                {/* Category Id */}
                 ...
                 <div>
                  ...
                  <input
                    type="text"
                    name="category"
                    placeholder="e.g. 64abc123def456ghi789012"
                    value={formik.values.category}
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    className={`w-full px-4 py-2.5 bg-[rgba(27,38,44,0.7)] rounded-xl text-[#bbe1fa] text-[13px] outline-none placeholder:text-[rgba(187,225,250,0.2)] border ${formik.touched.category && formik.errors.category ? "border-[rgba(187,225,250,0.4)]" : "border-[rgba(50,130,184,0.2)]"}`}
                  />
                  {formik.touched.category && formik.errors.category && <Err msg={formik.errors.category} />}
                </div>
      ...
      {showDeleteModal && selectedSubCategory && (
        <div onClick={() => setShowDeleteModal(false)} className="fixed inset-0 bg-[rgba(15,30,40,0.85)] flex items-center justify-center z-50 px-4">
          <div onClick={(e) => e.stopPropagation()} className="bg-[#0f4c75] border border-[rgba(50,130,184,0.25)] rounded-2xl p-6 w-full max-w-xs text-center">
            ...
            <div className="text-[15px] font-bold text-[#bbe1fa] mb-1.5">Delete Subcategory?</div>
            <div className="text-[13px] text-[rgba(187,225,250,0.4)] leading-relaxed mb-5">
              Are you sure you want to delete{" "}
              <span className="text-[#bbe1fa] font-semibold">{selectedSubCategory.name}</span>?
            </div>
            <div className="grid grid-cols-2 gap-2.5">
